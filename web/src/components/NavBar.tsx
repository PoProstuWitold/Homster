           ...
           <div className="relative w-full bg-white">
            ...
            <div className="md:hidden">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="p-5 flex font-extrabold">Homster</span>
                    ...
                    {/* :::Tab button */}
                    <div className="grid grid-cols-2 gap-x-4 border-b-primary border-b-2">
                    {navigation.categories.map(category => (
                        <button key={category.id} className={`col-span-1 py-3 px-4 ${category.id === activeTab ? "border-b-primary border-b-2 text-primary" : ""} text-center text-lg  font-semibold`} onClick={() => setActiveTab(category.id)}>{category.name}</button>
                    ))
                    }
                    </div>
                    {/* :::Tab content */}
                    ...
                    {navigation.categories
                        ...
                        <div key={category.id} className="grid grid-cols-2 gap-x-2 gap-y-4">
                            ...
                            <a key={item.name} href={item.href} className={`py-1 px-3 text-base ${index === 0 ? "text-primary" : "hover:text-primary"} font-semibold hover:underline`}>{item.name}</a>
                            ))
                            }
                        </div>
                        ))
                    ...
                    {/* :::Sign In / Sign Up */}
                    <div className="py-5 px-4 flex flex-col space-y-2 border-t-2">
                    <a href="#link" className="py-1 px-4 inline-flex items-center rounded-md text-base font-semibold tracking-wide btn btn-primary btn-ghost">
                        Sign In
                    </a>
                    <a href="#link" className="py-1 px-4 inline-flex items-center rounded-md text-base font-semibold tracking-wide btn btn-primary btn-ghost">
                        Sign Up
            ...
            <header className="relative">
                ...
                <div className="mx-auto max-w-6xl flex justify-between items-center">
                    ...
                    <div className="flex items-center">
                        ...
                        {/* ::::logo  */}
                        <a href="/" className="w-10 h-10 inline-flex justify-center items-center rounded-full">
                            ...
                            {navigation.categories.map(category => (
                                ...
                                <Popover>
                                    ...
                                    <>
                                        ...
                                        {category.name}
                                        <svg className={`ml-1.5 w-2.5 h-2.5 ${open ? "" : "text-primary group-hover:text-primary"} fill-current`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 21l-12-18h24z"/></svg>
                                        </Popover.Button>
                                        {/* :::Flyout menu */}
                    ...
                    {/* ::::user avatar */}
                    <a href="#gotoUserAccount" className="px-4 border-r-2 border-primary text-primary">
                        <AiOutlineUser className="w-6 h-6"/>
                    </a>
                    {/* ::::cart */}
                    <a href="#gotoUserCart" className="px-4 inline-flex items-center text-primary">
